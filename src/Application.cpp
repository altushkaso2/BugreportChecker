...
std::vector<fs::path> Application::findBugReports() const {
    ui_.showMessage("Searching for bugreport files...");
        ...
        try {
            ...
            for (const auto& entry : fs::directory_iterator(path)) {
                if (entry.is_regular_file()) {
                    ...
                    if (lower_filename.find("bugreport") != std::string::npos) {
...
void Application::handleAnalysis(bool isDebug, std::optional<fs::path> initial_path) {
    ...
    fs::path final_path = *selected_path_opt;
    ...
    std::cout << "Analyzing '" + final_path.string() + "'...\n";
    try {
        analyzer_.analyze(final_path, progress_bar, data);
        std::cout << std::endl;
